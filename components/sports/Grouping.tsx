...
const Grouping = ({ matches, groupedMatches, matchNumberMap, activeGame, maxWinAmount, onBetPlaced }: Props) => {
  ...
  const [groupSelections, setGroupSelections] = useState<Record<string, BetSelection[]>>({});
  ...
  const toggleBetForActive = (matchId: string, matchNumber: number, option: SportOptionKey, odds: number) => {
    if (!activeUId) {
    ...
    const withoutMatch = current.filter((b) => b.matchId !== matchId);
    setGroupSelections({
      ...groupSelections,
      [activeUId]: [...withoutMatch, { matchId, matchNumber, option, odds }],
    });
  };

  const isSelected = (matchId: string, option: SportOptionKey) => {
    if (!activeUId) return false;
    return (groupSelections[activeUId] ?? []).some((b) => b.matchId === matchId && b.option === option);
  };
  ...
  const winningsPreview = useMemo(() => {
    ...
    selectedUs.forEach((sel) => {
      ...
      (groupSelections[sel.id] ?? []).forEach((b) => {
        groups[key][String(b.matchNumber)] = [b.option];
  ...
  const placeBet = async () => {
    ...
    selectedUs.forEach((sel) => {
      ...
      selections[key] = {};
      (groupSelections[sel.id] ?? []).forEach((b) => {
        selections[key][String(b.matchNumber)] = [b.option];
      });
    });
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      <div className="lg:col-span-2 lg:col-start-1 lg:row-start-1 lg:row-span-2 space-y-3 order-1">
              ...
              >
                ...
                <div className="flex flex-wrap gap-1">
                  {(groupSelections[sel.id] ?? []).length === 0 ? (
                    <span className="text-xs text-muted-foreground">Select matches from table</span>
                  ) : (
      ...
      <div className="lg:col-span-3 lg:col-start-10 lg:row-start-2 flex flex-col gap-4 order-4">
        {isReady && (
          <div className="p-4 rounded-xl bg-card border border-border">
            ...
            <div className="text-xs text-muted-foreground text-center mt-1">
              {calcSportsGroupedApl(betAmount, storageGroups) > 0
                ? `${Math.round(betAmount / calcSportsGroupedApl(betAmount, storageGroups))} lines`
                : ""}
            </div>
          </div>
        )}
        ...
        <div className="p-4 rounded-xl bg-card border border-border">
          <label className="text-sm text-muted-foreground" htmlFor="grouping-bet-amount">
            Bet Amount
          </label>
