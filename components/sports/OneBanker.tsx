...
import SportsMatchTable from "./MatchTable";
import { BetSelection, optionLabels, SportsMatchRow } from "./types";
...
const OneBanker = ({ matches, groupedMatches, matchNumberMap, activeGame, maxWinAmount, onBetPlaced }: Props) => {
  ...
  const toggleBet = (matchId: string, matchNumber: number, option: SportOptionKey, odds: number) => {
    ...
    const without = selections.filter((b) => b.matchId !== matchId);
    setSelections([...without, { matchId, matchNumber, option, odds }]);
  };

  const isSelected = (matchId: string, option: SportOptionKey) =>
    selections.some((b) => b.matchId === matchId && b.option === option);
  ...
  const winningsPreview = useMemo(() => {
    ...
    groupABets.forEach((b) => {
      groups["1-groupA"][String(b.matchNumber)] = [b.option];
      oddsMap[String(b.matchNumber)] = b.odds;
    });
    groupBBets.forEach((b) => {
      groups["2-groupB"][String(b.matchNumber)] = [b.option];
      oddsMap[String(b.matchNumber)] = b.odds;
    });
  ...
  const placeBet = async () => {
    ...
    groupABets.forEach((b) => {
      selectionsObj["1-groupA"][String(b.matchNumber)] = [b.option];
    });
    groupBBets.forEach((b) => {
      selectionsObj["2-groupB"][String(b.matchNumber)] = [b.option];
    });
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      <div className="lg:col-span-2 lg:col-start-1 lg:row-start-1 lg:row-span-2 order-1">
        <div className="p-4 rounded-xl bg-card border border-border">
          ...
          <p className="text-xs text-muted-foreground mt-2">
            Banker must win. At least 2 of the remaining selections must win.
          </p>
        </div>
      </div>

      <div className="lg:col-span-7 lg:col-start-3 lg:row-start-1 lg:row-span-2 order-3 lg:order-2">
      ...
      <div className="lg:col-span-3 lg:col-start-10 lg:row-start-1 order-2 lg:order-3">
        <div className="p-4 rounded-xl bg-card border border-border">
          ...
          <div className="space-y-3">
            <div className="p-3 rounded-lg bg-primary/10 border border-primary">
              ...
              {groupABets.length === 0 ? (
                <p className="text-xs text-muted-foreground">Tap a selection below to set banker</p>
              ) : (
                groupABets.map((b) => (
                  <span key={b.matchId} className="px-2 py-1 rounded bg-card border text-xs">
                    {b.matchNumber} {optionLabels[b.option]} @{b.odds.toFixed(2)}
                  </span>
                ))
              )}
            </div>
            <div className="p-3 rounded-lg bg-muted/50 border border-border/50">
              ...
              {groupBBets.length === 0 ? (
                <p className="text-xs text-muted-foreground">Select 2+ more matches</p>
              ) : (
      ...
      <div className="lg:col-span-3 lg:col-start-10 lg:row-start-2 flex flex-col gap-4 order-4">
        {selections.length > 0 && (
          <div className="p-4 rounded-xl bg-card border border-border">
        ...
        <div className="p-4 rounded-xl bg-card border border-border">
          <label className="text-sm text-muted-foreground" htmlFor="banker-bet-amount">
            Bet Amount
          </label>
          <Input
            id="banker-bet-amount"
            type="number"
            min={0}
            value={betAmount}
