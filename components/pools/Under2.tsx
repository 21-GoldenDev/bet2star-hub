...
const Under2 = ({ gameMode, gameId, prizes, setGameMode, matches }: Props) => {
  ...
  const placeBet = async () => {
    if (selectedMatches.length !== 2) {
      toast.error("Select exactly 2 matches");
      return;
    }
    if (betAmount <= 0) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-2">
            ...
            <div className="flex flex-wrap gap-2">
              {matches.map((match) => (
                <button
                  ...
                  className={clsx(
                    "aspect-square w-12 rounded-xl font-bold text-lg cursor-pointer transition-all duration-300 disabled:text-gray-500 disabled:cursor-not-allowed",
        ...
        <div className="lg:col-span-3 flex flex-col gap-4">
          <div className="p-4 rounded-xl bg-card border border-border">
            ...
            <div className="space-y-3 overflow-y-auto scrollbar">
          ...
          <Button
            ...
            size="lg"
            onClick={placeBet}
            disabled={selectedMatches.length !== 2 || betAmount <= 0 || isPlacingBet}
            className="w-full py-3"
          >
            {isPlacingBet ? "Placing..." : "Stake"}
          </Button>

          {(selectedMatches.length !== 2 || betAmount <= 0) && (
            <div className="text-xs text-red-400 text-left list-disc ml-4">
              {selectedMatches.length !== 2 && <li>Select exactly 2 matches</li>}
