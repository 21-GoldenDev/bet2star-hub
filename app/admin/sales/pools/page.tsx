...
interface TabSummary {
  ...
  winByLabel: Record<string, number>;
  totalSale: number;
  totalPayableToAgents: number;
  totalWinning: number;
}
...
const buildSummary = (results: TerminalResult[]): TabSummary => {
  const salesByLabel: Record<string, number> = {};
  const winByLabel: Record<string, number> = {};
  let totalSale = 0;
  let totalPayableToAgents = 0;
  let totalWinning = 0;

  results.forEach((result) => {
    result.sales.forEach((sale) => {
      const label = sale.prize.name || "Unknown";
      salesByLabel[label] = (salesByLabel[label] || 0) + sale.amount;

      totalSale += sale.amount;
      totalPayableToAgents += (sale.amount * sale.prize.commission) / 100;
    });

    result.win.forEach((win) => {
      const label = win.prize || "Unknown";
      winByLabel[label] = (winByLabel[label] || 0) + win.amount;

      totalWinning += win.amount;
    });
  });
...
export default function PoolsSalesPage() {
  ...
  useEffect(() => {
    const fetchSalesData = async () => {
      ...
      try {
        setLoading(true);
        const res = await fetch(`/api/admin/sales/pools?game_id=${selectedWeek}`);
        if (res.ok) {
          const data = await res.json();
  ...
  const staffSummary = useMemo(() => buildSummary(staffFilteredResults), [staffFilteredResults]);
  const agentSummary = useMemo(() => buildSummary(agentFilteredResults), [agentFilteredResults]);
  const terminalSummary = useMemo(() => buildSummary(terminalFilteredResults), [terminalFilteredResults]);
  ...
  const terminalRows = useMemo(() => {
    return [...terminalFilteredResults]
      ...
      .map((result) => ({
        ...
        salesLines: result.sales.map((sale) => ({
          label: sale.prize.name,
          value: sale.amount,
        })),
        payableLines: result.sales.map((sale) => ({
          label: sale.prize.name,
          value: (sale.amount * sale.prize.commission) / 100,
        })),
        winLines: result.win.map((win) => ({
          label: win.prize,
          value: win.amount,
        })),
      }));
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap gap-4 justify-between items-center">
        <h1 className="text-3xl font-bold">Pools Sales Report</h1>
                    ...
                    <Command>
                      ...
                      <CommandList>
                        ...
                        <CommandGroup>
                          ...
                          >
                            <Check className={cn("mr-2 h-4 w-4", staffFilter === "all" ? "opacity-100" : "opacity-0")} />
                            All Staff
                          </CommandItem>
                          {staffOptions.map((staff) => (
                            ...
                            >
                              <Check className={cn("mr-2 h-4 w-4", staffFilter === staff ? "opacity-100" : "opacity-0")} />
                              {staff}
                            </CommandItem>
                          ))}
                        </CommandGroup>
            ...
            <Table className="text-center border">
              ...
              <TableBody>
                <TableRow>
                  ...
                  <TableCell className="border">
                    {Object.entries(gameTypes).map(([key, label]) => (
                      <div key={key} className="text-nowrap">{label}={(totalResult.win[key] || 0).toLocaleString()}</div>
                    ))}
                  </TableCell>
            ...
            <Table className="text-center border">
              ...
              <TableBody>
                <TableRow>
                  <TableCell className="border">
                    {Object.entries(gameTypes).map(([key, label]) => (
                      <div key={key} className="text-nowrap">{label}={(onlineResult.sales[key] || 0).toLocaleString()}</div>
                    ))}
                  </TableCell>
                  <TableCell className="border">{totalOnlineSales.toLocaleString()}</TableCell>
                  <TableCell className="border">
                    {Object.entries(gameTypes).map(([key, label]) => (
                      <div key={key} className="text-nowrap">{label}={(onlineResult.win[key] || 0).toLocaleString()}</div>
                    ))}
                  </TableCell>
          ...
          <TabsContent value="staff" className="space-y-4">
            <Table className="text-center border">
              <TableHeader>
                <TableRow>
                  <TableHead className="border text-center">Sales</TableHead>
                  <TableHead className="border text-center">Total Sale</TableHead>
                  <TableHead className="border text-center">Total Payable to Agents</TableHead>
                  <TableHead className="border text-center">Win</TableHead>
                  <TableHead className="border text-center">Total Winning</TableHead>
                  <TableHead className="border text-center">Bal Company</TableHead>
                  <TableHead className="border text-center">Status</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                <TableRow>
                  <TableCell className="border">
                    {Object.entries(staffSummary.salesByLabel).map(([label, value]) => (
                      <div key={label} className="text-nowrap">{label}={value.toLocaleString()}</div>
                    ))}
                    {!Object.keys(staffSummary.salesByLabel).length && <div>-</div>}
                  </TableCell>
                  <TableCell className="border">{staffSummary.totalSale.toLocaleString()}</TableCell>
                  <TableCell className="border">{staffSummary.totalPayableToAgents.toLocaleString()}</TableCell>
                  <TableCell className="border">
                    {Object.entries(staffSummary.winByLabel).map(([label, value]) => (
                      <div key={label} className="text-nowrap">{label}={value.toLocaleString()}</div>
                    ))}
                    {!Object.keys(staffSummary.winByLabel).length && <div>-</div>}
                  </TableCell>
                  <TableCell className="border">{staffSummary.totalWinning.toLocaleString()}</TableCell>
                  <TableCell className="border">{(staffSummary.totalSale - staffSummary.totalWinning).toLocaleString()}</TableCell>
                  <TableCell className="border">
                    {staffSummary.totalSale - staffSummary.totalWinning > 0 ? (
                      <Badge className="bg-green-600 hover:bg-green-700">Green</Badge>
                    ) : (
                      <Badge variant="destructive">Red</Badge>
                    )}
          ...
          <TabsContent value="agent" className="space-y-4">
            <Table className="text-center border">
              ...
              <TableBody>
                <TableRow>
                  <TableCell className="border">
                    {Object.entries(agentSummary.salesByLabel).map(([label, value]) => (
                      <div key={label} className="text-nowrap">{label}={value.toLocaleString()}</div>
                    ))}
                    {!Object.keys(agentSummary.salesByLabel).length && <div>-</div>}
                  </TableCell>
                  <TableCell className="border">{agentSummary.totalSale.toLocaleString()}</TableCell>
                  <TableCell className="border">{agentSummary.totalPayableToAgents.toLocaleString()}</TableCell>
                  <TableCell className="border">
                    {Object.entries(agentSummary.winByLabel).map(([label, value]) => (
                      <div key={label} className="text-nowrap">{label}={value.toLocaleString()}</div>
                    ))}
                    {!Object.keys(agentSummary.winByLabel).length && <div>-</div>}
                  </TableCell>
                  <TableCell className="border">{agentSummary.totalWinning.toLocaleString()}</TableCell>
                  <TableCell className="border">{(agentSummary.totalSale - agentSummary.totalWinning).toLocaleString()}</TableCell>
                  <TableCell className="border">
                    {agentSummary.totalSale - agentSummary.totalWinning > 0 ? (
                      <Badge className="bg-green-600 hover:bg-green-700">Green</Badge>
                    ) : (
                      <Badge variant="destructive">Red</Badge>
                    )}
          ...
          <TabsContent value="terminal" className="space-y-4">
            <Table className="text-center border">
              ...
              <TableBody>
                <TableRow>
                  <TableCell className="border">
                    {Object.entries(terminalSummary.salesByLabel).map(([label, value]) => (
                      <div key={label} className="text-nowrap">{label}={value.toLocaleString()}</div>
                    ))}
                    {!Object.keys(terminalSummary.salesByLabel).length && <div>-</div>}
                  </TableCell>
                  <TableCell className="border">{terminalSummary.totalSale.toLocaleString()}</TableCell>
                  <TableCell className="border">{terminalSummary.totalPayableToAgents.toLocaleString()}</TableCell>
                  <TableCell className="border">
                    {Object.entries(terminalSummary.winByLabel).map(([label, value]) => (
                      <div key={label} className="text-nowrap">{label}={value.toLocaleString()}</div>
                    ))}
                    {!Object.keys(terminalSummary.winByLabel).length && <div>-</div>}
                  </TableCell>
                  <TableCell className="border">{terminalSummary.totalWinning.toLocaleString()}</TableCell>
                  <TableCell className="border">{(terminalSummary.totalSale - terminalSummary.totalWinning).toLocaleString()}</TableCell>
                  <TableCell className="border">
                    {terminalSummary.totalSale - terminalSummary.totalWinning > 0 ? (
                      <Badge className="bg-green-600 hover:bg-green-700">Green</Badge>
                    ) : (
                      <Badge variant="destructive">Red</Badge>
                    )}
                      ...
                      terminals.forEach((row, terminalIdx) => {
                        ...
                        rows.push(
                          <TableRow key={`${staff}-${agent}-${row.terminal}-${terminalIdx}`}>
                            ...
                            <TableCell className="border">
                              {row.salesLines.reduce((sum, sale) => sum + sale.value, 0) - row.winLines.reduce((sum, win) => sum + win.value, 0) > 0 ? (
                                <Badge className="bg-green-600 hover:bg-green-700">Green</Badge>
                              ) : (
                                <Badge variant="destructive">Red</Badge>
                              )}
