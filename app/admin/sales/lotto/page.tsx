...
export default function LottoSalesPage() {
  ...
  useEffect(() => {
    const fetchWeeks = async () => {
      try {
        const res = await fetch("/api/admin/bets/lotto/weeks");
        if (res.ok) {
  ...
  useEffect(() => {
    const fetchSalesData = async () => {
      ...
      try {
        setLoading(true);
        const res = await fetch(`/api/admin/sales/lotto?game_id=${selectedWeek}`);
        if (res.ok) {
          const data = await res.json();
