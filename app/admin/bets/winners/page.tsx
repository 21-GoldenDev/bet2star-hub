...
export default function WinnersPage() {
  ...
  const [lottoBets, setLottoBets] = useState<WinningBet[]>([]);
  const [poolsBets, setPoolsBets] = useState<WinningBet[]>([]);
  const [dailyPoolsBets, setDailyPoolsBets] = useState<WinningBet[]>([]);
  const [sportsBets, setSportsBets] = useState<WinningBet[]>([]);
  const [sportsDrawBets, setSportsDrawBets] = useState<WinningBet[]>([]);
  const [dataMatches, setDataMatches] = useState<Record<string, MatchInfo[]>>({});
  const [loading, setLoading] = useState(true);
  const [isDetailsOpen, setIsDetailsOpen] = useState(false);
  const [selectedBet, setSelectedBet] = useState<{ bet: WinningBet; type: string } | null>(null);
  ...
  const matchesDateRange = (bet: any) => {
    ...
    const timeIso = bet?.betTime ?? bet?.bet_time ?? bet?.betTimeIso;
  ...
  async function fetchWinningBets() {
    setLoading(true);
    try {
      const [lottoRes, poolsRes, dailyPoolsRes, sportsRes, sportsDrawRes] = await Promise.all([
        fetch("/api/admin/bets/lotto/winners"),
        fetch("/api/admin/bets/pools/winners"),
        fetch("/api/admin/bets/pools/winners?type=daily_pools"),
        fetch("/api/admin/bets/sports/winners"),
        fetch("/api/admin/bets/sports-draw/winners"),
      ]);
      ...
      toast({
        title: "Error",
        description: "Failed to fetch winning bets.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  function getTerminalLabel(terminal: WinningBet["terminal"] | string | undefined) {
    if (!terminal) return "—";
    if (typeof terminal === "string") return terminal;
  ...
  function calculateAplForWinningBet(row: WinningBet) {
  ...
  const renderActions = (row: WinningBet, type: string) => (
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-6">
          ...
          <DataTable
            ...
            columns={[
              ...
              },
              {
                key: "id",
                label: "APL",
                render: (_: string, row: WinningBet) => calculateAplForWinningBet(row).toFixed(2),
              },
              { key: "staked", label: "Staked", render: (value: number) => value.toFixed(0) },
              { key: "award", label: "Winning", render: (value: number) => (
                <span className="text-red-600 font-medium">{(value ?? 0).toFixed(2)}</span>
              ) },
        ...
        <TabsContent value="daily-pools" className="mt-4">
          <DataTable
            title="Winning Daily/Mid-week Pools Bets"
        ...
        <TabsContent value="sports-draw" className="mt-4">
          <DataTable
            title="Winning Football Pool Bets"
                    ...
                    <div className="border-t pt-4 grid grid-cols-2 gap-4">
                      ...
                      {!isSportsType && (
                        <div>
                          ...
                          <p className="mt-1 font-medium text-lg">{calculateAplForWinningBet(bet).toFixed(2)}</p>
                        </div>
                      )}
                      {isSportsType && (
                        <div>
                          <Label className="text-xs font-semibold text-muted-foreground">Award</Label>
                          <p className="mt-1 font-medium text-lg text-red-600">{(bet.award ?? 0).toFixed(2)}</p>
                        </div>
                      )}
                    </div>
