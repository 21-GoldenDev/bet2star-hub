...
export default function DeletedBetsPage() {
  ...
  async function fetchDeletedBets() {
    setLoading(true);
    try {
      const [lottoRes, poolsRes, sportsRes] = await Promise.all([
        fetch("/api/admin/bets/lotto/deleted"),
        fetch("/api/admin/bets/pools/deleted"),
        fetch("/api/admin/bets/sports/deleted"),
      ]);
      ...
      toast({
        title: "Error",
        description: "Failed to fetch deleted bets.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  async function voidBet() {
    ...
    try {
      const response = await fetch(`/api/admin/bets/${selectedBet.type}/void`, {
        method: "POST",
      ...
      toast({
        title: "Error",
        description: "Failed to void bet.",
        variant: "destructive",
      });
    } finally {
    ...
    return (
      <div className="p-6">
        ...
        <div className="mt-12 flex items-center justify-center">
          <div className="text-muted-foreground">Loading deleted bets...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6">
      ...
      <p className="text-sm text-muted-foreground mt-2">
        Manage bets that have been deleted by players or agents.
      </p>
      ...
      <AlertDialog open={isDeleteAlertOpen} onOpenChange={setIsDeleteAlertOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Do you really want to void bet?</AlertDialogTitle>
          </AlertDialogHeader>
          <AlertDialogFooter>
